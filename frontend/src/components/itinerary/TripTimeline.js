...
export default function TripTimeline({
  ...
  // Hide legacy transfer items created by the old backend (category=transport
  // with name starting with "Transfer:"). New builds don't inject these
  // anymore; this filter keeps old trips clean without requiring a rebuild.
  const isLegacyTransfer = (it) => {
    const cat = (it?.category || "").toString().toLowerCase();
    const name = (it?.name || "").toString();
    return cat === "transport" && /^transfer[:\s]/i.test(name);
  };
  const items = (day.itinerary_items || []).filter((it) => !isLegacyTransfer(it));
